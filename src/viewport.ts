...
export class ViewportFunctions
{
     ...
     */
    static async CenterViewportOnImage(iii: IItemImage)
    {
        ...
        const imageCenter = await this.GetImageCenter(iii, dpi);
     ...
     */
    static async GetImageCenter(iii: IItemImage, dpi: number)
    {
        const dpiScale = dpi / iii.dpi;
        const width = iii.width * dpiScale;
        const height = iii.height * dpiScale;
        const offsetX = (iii.offsetx / iii.width) * width;
        const offsetY = (iii.offsety / iii.height) * height;
        return {
            x: iii.xpos - offsetX + width / 2,
            y: iii.ypos - offsetY + height / 2,
        };
    }
}
